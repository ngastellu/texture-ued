...
inp_file = 'Project3_orientations_rad_newest.dat'
...
#max_I_exptl = simps(exptl_pattern,exptl_s)
...
#plt.plot(sgrid/(4.0*np.pi),textured_out/max_I,linewidth=0.9,label='textured',c='r')
...
#plt.plot(sgrid/(4.0*np.pi),(textured_out-ideal_out)/np.max(np.abs(ideal_out-textured_out)),linewidth=0.9,label='difference',c='r')
#plt.plot(exptl_s[82:]/(4.0*np.pi),exptl_pattern[82:]/max_I_exptl,linewidth=0.9,label='experimental',c='#67d540')
...
#plt.savefig('../data/no_texture_with_broadening.png')
plt.show()
